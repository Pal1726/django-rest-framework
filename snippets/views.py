...
# # normal httprequest and httpresponse(regular django view)
...
#         serializer = SnippetSerializer(snippets, many=True)  # Serialize multiple snippets
...
#         serializer = SnippetSerializer(data=data)  # Deserialize data
#         if serializer.is_valid():  # Validate data
#             serializer.save()  # Save snippet to the database
#             return JsonResponse(serializer.data, status=201)  # Return success response
#         return JsonResponse(serializer.errors, status=400)  # Return validation errors

# @csrf_exempt
# def snippet_detail(request, pk):
#     """
#     Retrieve, update, or delete a snippet by ID (pk).
#     """
#     try:
#         snippet = Snippet.objects.get(pk=pk)  # Fetch snippet by ID
#     except Snippet.DoesNotExist:
#         return HttpResponse(status=404)  # Return 404 if not found

#     if request.method == 'GET':  # Handle GET requests
#         serializer = SnippetSerializer(snippet)  # Serialize single snippet
#         return JsonResponse(serializer.data)  # Return JSON response

#     elif request.method == 'PUT':  # Handle PUT requests
#         data = JSONParser().parse(request)  # Parse incoming JSON data
#         serializer = SnippetSerializer(snippet, data=data)  # Deserialize and update
#         if serializer.is_valid():  # Validate data
#             serializer.save()  # Save changes
#             return JsonResponse(serializer.data)  # Return success response
#         return JsonResponse(serializer.errors, status=400)  # Return validation errors

#     elif request.method == 'DELETE':  # Handle DELETE requests
#         snippet.delete()  # Delete snippet
#         return HttpResponse(status=204)  # Return "no content" status


#request and responses(function view)
...
#instead of userlist and userdetail we can directly use userviewset
